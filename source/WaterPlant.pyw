...
class Shopee():
    def __init__(self):
        ...
        self.url = {
            ...
            'shopee-farm-water':'https://games.shopee.sg/farm/api/orchard/crop/water',
            'shopee-grocery-store':'https://games.shopee.sg/farm/api/grocery_store/claim'
        }
        self.locator = {
    ...
    def getWater(self, user):
        cookies = {}
        for cookie in pickle.load(open(path.cookies + user, 'rb')): cookies[cookie['name']] = cookie['value']
        res = json.loads(requests.post(self.url['shopee-farm-water'], cookies=cookies).text)
        logging.info(f'User[{user}] has watered the plants.')
        #print (res['data'])
        #if res['data']['success'] == True: logging.info(f'User[{user}] watered the plans.')
        #else: logging.info(f'User[{user}] already watered the plants. (Or failed to)')
    # end getCoins()
    
    def getGrocery(self, user):
        cookies = {}
        for cookie in pickle.load(open(path.cookies + user, 'rb')): cookies[cookie['name']] = cookie['value']
...
class PathManager():
    def __init__(self):
        ...
        self.gsc = self.documents + r'\ShopeeFarm'
